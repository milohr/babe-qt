...
Pulpo::Pulpo(const QString &title_, const QString &artist_, const QString &album_, QObject *parent) : QObject(parent)
    ,album(BaeUtils::fixString(album_))
    ,artist(BaeUtils::fixString(artist_))
    ,title(BaeUtils::fixString(title_))
{
}
...
bool Pulpo::fetchArtistInfo(const ArtistInfo &infoType, const InfoServices &service, const bool &recursive)
{
    ...
    if (!this->artist.isEmpty()) {
        ...
        case LastFm:
            ...
            if (!array.isEmpty())
                return lastfm.parseLastFmArtist(array, infoType);
            return false;
        case Spotify:
            array = startConnection(spotify.setUpService(spotify::ARTIST));
            if (!array.isEmpty())
                return spotify.parseSpotifyArtist(array, infoType);
            return false;
        case iTunes:
            break;
        case infoCRAWL:
            break;
        case AllInfoServices:
            break;
        }
    }
    return false;
...
bool Pulpo::fetchAlbumInfo(const AlbumInfo &infoType, const InfoServices &service, const bool &recursive)
{
    ...
    if (!this->artist.isEmpty() && !this->album.isEmpty()) {
        ...
        case LastFm:
            ...
            if (!array.isEmpty() && lastfm.parseLastFmAlbum(array, infoType))
                return true;
            return false;
        case Spotify:
            array = startConnection(spotify.setUpService(spotify::ALBUM));
            if (!array.isEmpty() && spotify.parseSpotifyAlbum(array, infoType))
                    return true;
            return false;
        case iTunes:
            break;
...
bool Pulpo::fetchTrackInfo(const Pulpo::TrackInfo &infoType, const Pulpo::LyricServices &lyricService, const Pulpo::InfoServices &service)
{
    ...
    if (!this->artist.isEmpty() && !this->album.isEmpty()) {
        ...
        case LastFm:
            ...
            if (!array.isEmpty())
                return lastfm.parseLastFmTrack(array, infoType);
            return false;
        case Spotify:
            array = startConnection(spotify.setUpService(spotify::ARTIST));
            if (!array.isEmpty())
                return spotify.parseSpotifyTrack(array, infoType);
            return false;
        case iTunes:
            break;
        case infoCRAWL:
