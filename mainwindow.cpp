    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(this,SIGNAL(fetchCover(QString,QString,QString)),this,SLOT(setCoverArt(QString,QString,QString)));
...
{
    QList<QMap<int, QString>> mapList = readTrackData(info[Album::ALBUM], info[Album::ARTIST]);
    if (!mapList.isEmpty())
        addToPlaylist(mapList);
}

QList<QMap<int,QString>> MainWindow::readTrackData(const QString &album, const QString &artist)
{
    QList<QMap<int, QString>> mapList;
    QVariantMap filter;
    filter.insert("artist", artist);
    if (album.isEmpty()) {
        mapList = settings_widget->getCollectionDB().getTrackData(filter, QString("album asc, track asc"));
    } else if (!album.isEmpty() && !artist.isEmpty()) {
        filter.insert("album", album);
        mapList = settings_widget->getCollectionDB().getTrackData(filter, QString("track asc"));
    }
    return mapList;
}
...
{
    if (!info.isEmpty()) {
        qDebug() << "put on play<< " << info[Album::ARTIST] << info[Album::ALBUM];
        QList<QMap<int, QString>> mapList = readTrackData(info[Album::ALBUM], info[Album::ARTIST]);
        if (!mapList.isEmpty()) {
            mainList->flushTable();
            currentList.clear();
...
{
    QList<QMap<int, QString>> list = settings_widget->getCollectionDB().getTrackData(url);
    if (list.size() > 0) {
        int played = 0;
        QMap<int, QString> map;
        foreach (map, list)
            played = map.value(BabeTable::PLAYED).toInt();
        QVariantMap where;
        where.insert("location", url);
        settings_widget->getCollectionDB().updateTrack("played", url, played+1);
    }
}
...
void MainWindow::dropEvent(QDropEvent *event)
{
    ...
    if (urls.isEmpty()) {
        ...
        QVariantMap filter;
        filter.insert("artist", "");
        if (infoList.size()==2) {
            QString _artist = infoList.at(1).simplified();
            QString _album = infoList.at(0).simplified();
            filter.insert("artist", _artist);
            filter.insert("album", _album);
            mapList = settings_widget->collection_db.getTrackData(filter, QString("track asc"));
        } else {
            filter.insert("artist", info);
            mapList = settings_widget->collection_db.getTrackData(filter, QString("album asc, track asc"));
        }
        addToPlaylist(mapList);
    } else {
        QList<QUrl> urls = event->mimeData()->urls();
...
{
    QVariantMap filter;
    filter.insert("babe", 1);
    auto results = settings_widget->collection_db.getTrackData(filter, QString("played desc"));
...
bool MainWindow::isBabed(QMap<int, QString> track)
{
    if (track.isEmpty())
        return false;
    QVariantMap filter;
    filter.insert("location", track[BabeTable::LOCATION]);
    filter.insert("babe", 1);
    QList<QMap<int, QString>> tracks = settings_widget->getCollectionDB().getTrackData(filter);
    return (!tracks.isEmpty() && !tracks.at(0).isEmpty());
}

void MainWindow::loadMood()
{
    QString color;
    QList<QMap<int, QString>> tracks = settings_widget->collection_db.getTrackData(current_song[BabeTable::LOCATION]);
    QMap<int, QString> map;
    foreach (map, tracks)
        color = map.value(BabeTable::ART);
    if (!color.isEmpty()) {
...
bool MainWindow::loadCover(QString artist, QString album, QString title) //tofix separte getalbumcover from get artisthead
{
    ...
    if (settings_widget->getCollectionDB().checkQuery("SELECT * FROM tracks WHERE location = \""+current_song[BabeTable::LOCATION]+"\"")) {
        ...
        if (settings_widget->collection_db.checkQuery("SELECT * FROM albums WHERE title = \""+album+"\" AND artist = \""+artist+"\"")) {
            artistHead = settings_widget->getCollectionDB().getArtistArt(artist);
            albumCover = settings_widget->getCollectionDB().getAlbumArt(album, artist);
            if(!albumCover.isEmpty())
                album_art->putPixmap(albumCover);
...
void MainWindow::babeAlbum(QMap<int,QString> info)
{
    ...
    if (!artist.isEmpty())
        filter.insert("album", album);
    mapList = settings_widget->getCollectionDB().getTrackData(filter, "");
    if (!mapList.isEmpty())
...
{
    if (settings_widget->getCollectionDB().updateTrack("babe", track[BabeTable::LOCATION], 0)) {
...
bool MainWindow::babeIt(QMap<int, QString> track)
{
    ...
    if (isBabed(track)) {
        ...
        if (settings_widget->getCollectionDB().hasTrack("location", url)) {
            if (settings_widget->getCollectionDB().updateTrack("babe", url, 1)) {
...
void MainWindow::scanNewDir(QString url,QString babe)
{
    ...
    while (it.hasNext()) {
        ...
        qDebug() << song;
        if (!settings_widget->getCollectionDB().hasTrack("location", song))
            list << song;
    }
    if (!list.isEmpty()) {
...
bool MainWindow::addToCollectionDB(QStringList url, QString babe)
{
    if (settings_widget->getCollectionDB().addTrack(url, babe.toInt())) {
        if (babe.contains("1"))
            for (auto track : url)
                addToPlaylist(settings_widget->getCollectionDB().getTrackData(track), true);
        return true;
    }
    return false;
}
    ...
    for (auto searchQuery : queries) {
        ...
        if (!searchQuery.isEmpty()) {
            ...
            if (key == "location:") {
                ...
                mapList += settings_widget->collection_db.getTrackData(filter, "", "LIKE");
            } else if (key == "artist:") {
                ...
                filter.insert("genre", searchQuery);
                filter.insert("title", searchQuery);
                mapList += settings_widget->collection_db.getTrackData(filter, "", "OR", "LIKE");
            }
        }
    }
    return mapList;
...
void MainWindow::saveResultsTo(QAction *action)
{
    ...
    switch (views->currentIndex()) {
        case COLLECTION:
            collectionTable->populatePlaylist(collectionTable->getTableContent(BabeTable::LOCATION), playlist);
            break;
        case ALBUMS:
            albumsTable->albumTable->populatePlaylist(albumsTable->albumTable->getTableContent(BabeTable::LOCATION), playlist);
            break;
        case ARTISTS:
            artistsTable->albumTable->populatePlaylist(artistsTable->albumTable->getTableContent(BabeTable::LOCATION), playlist);
            break;
        case PLAYLISTS:
            playlistTable->table->populatePlaylist(playlistTable->table->getTableContent(BabeTable::LOCATION), playlist);
            break;
        case RESULTS:
            resultsTable->populatePlaylist(resultsTable->getTableContent(BabeTable::LOCATION), playlist);
            break;
    }
}
